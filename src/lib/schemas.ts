...
const NuevoIncidenteSchema = z.object({
...
type NuevoIncidenteInput = z.infer<typeof NuevoIncidenteSchema>;
...
const NotaTurnoSchema = z.object({
  id: z.string().min(1),
  texto: z.string().min(1, 'La nota no puede estar vacía'),
  autor: z.string().min(1),
  timestamp: z.number().positive(),
...
type TurnoInput = z.infer<typeof TurnoSchema>;
...
const validateTurno = (data: unknown) => TurnoSchema.safeParse(data);
const validateNuevoIncidente = (data: unknown) => NuevoIncidenteSchema.safeParse(data);

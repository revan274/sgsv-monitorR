...
import { PCP_TERMINO_OPTIONS, DEFAULT_PCP_TERMINO, normalizeSearchText, createId, safeText, MAX_PCP_IMAGES } from '../lib/utils';
...
export default function PcpView({
  ...
  const [pcpForm, setPcpForm] = useState<PcpForm>({
    id: createId(), nombre: '', terminos: DEFAULT_PCP_TERMINO, descripcion: '', imagenes: []
  });
  ...
  const handlePCPImageUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const startIndex = pcpForm.imagenes.length;
      const images = await Promise.all(filesToAdd.map((file, index) =>
        handleMediaFile({
          file,
          storagePath: `pcp/${pcpForm.id}/${Date.now()}_${startIndex + index}.jpg`,
          entityType: 'pcp',
          entityId: pcpForm.id,
          field: 'imagenes',
          imagenesIndex: startIndex + index,
        }),
      ));
  ...
  const savePCP = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setPcpForm({ id: createId(), nombre: '', terminos: DEFAULT_PCP_TERMINO, descripcion: '', imagenes: [] });
          ...
          <form onSubmit={savePCP} className="grid md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <select value={pcpForm.terminos} onChange={e => setPcpForm({...pcpForm, terminos: e.target.value})} className="glass-input w-full rounded-lg p-3 text-white">
                {allPcpTerminos.map(t => <option key={t} value={t} className="bg-slate-900">{t}</option>)}
              </select>

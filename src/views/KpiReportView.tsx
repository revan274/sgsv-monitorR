...
export default function KpiReportView({ incidentes, personasInteres }: KpiReportViewProps) {
  ...
  const allIncidentTypes = useMemo(
    () => Array.from(new Set([
      ...INCIDENT_TYPES,
      ...incidentes.map((inc) => String(inc.tipo || '')).filter(Boolean),
    ])),
    [incidentes],
  );
  ...
  const reportData = useMemo(() => {
    ...
    allIncidentTypes.forEach((t) => { byType[t] = 0; });
    ...
    incidentesPeriodo.forEach((inc) => {
      ...
      if (!chartDataByDate[dk]) {
        ...
        allIncidentTypes.forEach((t) => { chartDataByDate[dk][t] = 0; });
      }
  ...
  }, [incidentes, dateRange, now, allIncidentTypes]);

...
const EditIncidentModal = ({ isOpen, onClose, incidente, updateIncidente, notify }: EditIncidentModalProps) => {
  ...
  const allIncidentTypes = Array.from(new Set([
    ...INCIDENT_TYPES,
    ...(config.customIncidentTypes || []),
    formData.tipo,
  ].filter(Boolean)));
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              >
                {allLocations.map(loc => <option key={loc} value={loc} className="bg-slate-800">{loc}</option>)}
              </select>
            </div>
            ...
            <div className="space-y-1">
              ...
              >
                {allIncidentTypes.map(tipo => <option key={tipo} value={tipo} className="bg-slate-800">{tipo}</option>)}
              </select>
            </div>
